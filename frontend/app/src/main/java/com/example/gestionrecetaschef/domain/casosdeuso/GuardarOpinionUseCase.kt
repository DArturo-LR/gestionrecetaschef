...
class GuardarOpinionUseCase(private val repositorio: RecetasRepositorio = RecetasRepositorio()
) {
    suspend operator fun invoke(opinion: OpinionRequest) = repositorio.guardarOpinion(opinion)
}